...
async function getItems(categoria){           
    ...
    try{
        var query;
        if(categoria){
            query = await conn.query('select itemid, status, categoria, descricao, data from item where aprovado=true and categoria=$1 except SELECT d.itemid,i.status,  i.categoria, i.descricao, i.data from item i INNER JOIN devolucao d on d.itemid = i.itemid order by itemid;',[categoria])
        } else {
            query = await conn.query('select itemid, status, categoria, descricao, data from item  where aprovado=true except SELECT d.itemid,i.status,  i.categoria, i.descricao, i.data from item i INNER JOIN devolucao d on d.itemid = i.itemid order by itemid;')//retorno do BD e guarda em uma variavel
        }
